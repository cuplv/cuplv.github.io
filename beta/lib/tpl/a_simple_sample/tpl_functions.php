...
function render_full_navigation($menuid){
...
function render_browser_navigation($naviID) {
	$navigation = generateLinks($naviID);
	foreach($navigation as $level => $items) {
		print '	<div><ul id="mainnav" class="nav">';
		foreach($items as $pid => $link) {
			print '<li>' . $link . '</li>';
		}
		print '	</ul></div>';
	}
...
function generateLinks($naviID) {
    ...
    foreach((array) $data as $pid => $info){
		...
		if(!is_array($navigation[$info['lvl']])) $navigation[$info['lvl']] = array();
